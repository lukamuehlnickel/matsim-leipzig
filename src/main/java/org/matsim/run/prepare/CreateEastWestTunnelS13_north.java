...
public class CreateEastWestTunnelS13_north {
    ...
    public static void main (String[]args){
        ...
        var Sellerhausen = network.getNodes().get(Id.createNodeId("pt_008080930"));
        var Paunsdorf = network.getNodes().get(Id.createNodeId("pt_008010208"));
        ...
        var connect8_we = createLink("pt_4_we", Hbf, FriedrichListPlatz);
        var connect9_we = createLink("pt_5_we", FriedrichListPlatz, LeipzigOst);
        var connect10_we = createLink("pt_6_we", LeipzigOst, Sellerhausen);
        var connect11_we = createLink("pt_7_we",Sellerhausen , Paunsdorf);
        var connect12_we = createLink("pt_8_we", Paunsdorf,Engelsdorf);
        var connect13_we = network.getLinks().get(Id.createLinkId("pt_10867"));
        var connect14_we = network.getLinks().get(Id.createLinkId("pt_11360"));
        var connect15_we = network.getLinks().get(Id.createLinkId("pt_11361"));
        var connect16_we = network.getLinks().get(Id.createLinkId("pt_11362"));
        var connect17_we = network.getLinks().get(Id.createLinkId("pt_11363"));
        var connect18_we = network.getLinks().get(Id.createLinkId("pt_11364"));
        ...
        network.addLink(connect11_we);
        network.addLink(connect12_we);
        ...
        var connect7_ew = createLink("pt_2_ew", Engelsdorf, Paunsdorf);
        var connect8_ew = createLink("pt_3_ew", Paunsdorf, Sellerhausen);
        var connect9_ew = createLink("pt_4_ew", Sellerhausen, LeipzigOst);
        var connect10_ew = createLink("pt_5_ew", LeipzigOst, FriedrichListPlatz);
        var connect11_ew = createLink("pt_6_ew", FriedrichListPlatz, Hbf);
        var connect12_ew = createLink("pt_7_ew", Hbf, Sportforum);
        var connect13_ew = createLink("pt_8_ew", Sportforum, Lindenauer);
        var connect14_ew = createLink("pt_9_ew", Lindenauer, Plagwitz);
        var connect15_ew = network.getLinks().get(Id.createLinkId("pt_11266"));
        var connect16_ew = network.getLinks().get(Id.createLinkId("pt_11267"));
        var connect17_ew = network.getLinks().get(Id.createLinkId("pt_11268"));
        var connect18_ew = network.getLinks().get(Id.createLinkId("pt_11269"));
        var endLink_ew = createLink("pt_end_ew", MilitzerAllee, startNode);
        network.addLink(connect7_ew);
        ...
        var stop9_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-FriedrichListPlatz_we", TransitStopFacility.class), FriedrichListPlatz.getCoord(), false);
        stop9_facility_we.setLinkId(connect8_we.getId());

        var stop10_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-LeipzigOst_we", TransitStopFacility.class), LeipzigOst.getCoord(), false);
        stop10_facility_we.setLinkId(connect9_we.getId());

        var stop11_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Sellerhausen_we", TransitStopFacility.class), Sellerhausen.getCoord(), false);
        stop11_facility_we.setLinkId(connect10_we.getId());

        var stop12_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Paunsdorf_we", TransitStopFacility.class), Paunsdorf.getCoord(), false);
        stop12_facility_we.setLinkId(connect11_we.getId());

        var stop13_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Engelsdorf_we", TransitStopFacility.class), Engelsdorf.getCoord(), false);
        stop13_facility_we.setLinkId(connect12_we.getId());

        var stop14_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Borsdorf_we", TransitStopFacility.class), Borsdorf.getCoord(), false);
        stop14_facility_we.setLinkId(connect13_we.getId());

        var stop15_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Gerichshain_we", TransitStopFacility.class), Gerichshain.getCoord(), false);
        stop15_facility_we.setLinkId(connect14_we.getId());

        var stop16_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Machern_we", TransitStopFacility.class), Machern.getCoord(), false);
        stop16_facility_we.setLinkId(connect15_we.getId());

        var stop17_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Altenbach_we", TransitStopFacility.class), Altenbach.getCoord(), false);
        stop17_facility_we.setLinkId(connect16_we.getId());

        var stop18_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Bennewitz_we", TransitStopFacility.class), Bennewitz.getCoord(), false);
        stop18_facility_we.setLinkId(connect17_we.getId());

        var stop19_facility_we = scheduleFactory.createTransitStopFacility(Id.create("Wurzen_we", TransitStopFacility.class), Wurzen.getCoord(), false);
        stop19_facility_we.setLinkId(connect18_we.getId());
        ...
        var stop8_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Paunsdorf_ew", TransitStopFacility.class), Paunsdorf.getCoord(), false);
        stop8_facility_ew.setLinkId(connect7_ew.getId());

        var stop9_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Sellerhausen_ew", TransitStopFacility.class), Sellerhausen.getCoord(), false);
        stop9_facility_ew.setLinkId(connect8_ew.getId());

        var stop10_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-LeipzigOst_ew", TransitStopFacility.class), LeipzigOst.getCoord(), false);
        stop10_facility_ew.setLinkId(connect9_ew.getId());

        var stop11_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-FriedrichListPlatz_ew", TransitStopFacility.class), FriedrichListPlatz.getCoord(), false);
        stop11_facility_ew.setLinkId(connect10_ew.getId());

        var stop12_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Hbf_ew", TransitStopFacility.class), Hbf.getCoord(), false);
        stop12_facility_ew.setLinkId(connect11_ew.getId());

        var stop13_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Sportforum_ew", TransitStopFacility.class), Sportforum.getCoord(), false);
        stop13_facility_ew.setLinkId(connect12_ew.getId());

        var stop14_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Lindenauer_ew", TransitStopFacility.class), Lindenauer.getCoord(), false);
        stop14_facility_ew.setLinkId(connect13_ew.getId());

        var stop15_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Plagwitz_ew", TransitStopFacility.class), Plagwitz.getCoord(), false);
        stop15_facility_ew.setLinkId(connect14_ew.getId());

        var stop16_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Grünauer Allee_ew", TransitStopFacility.class), GruenauerAllee.getCoord(), false);
        stop16_facility_ew.setLinkId(connect15_ew.getId());

        var stop17_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Allee-Center_ew", TransitStopFacility.class), AlleeCenter.getCoord(), false);
        stop17_facility_ew.setLinkId(connect16_ew.getId());

        var stop18_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Karlsruher Str._ew", TransitStopFacility.class), Karlsruherstr.getCoord(), false);
        stop18_facility_ew.setLinkId(connect17_ew.getId());

        var stop19_facility_ew = scheduleFactory.createTransitStopFacility(Id.create("Leipzig-Militzer Allee_ew", TransitStopFacility.class), startNode.getCoord(), false);
        stop19_facility_ew.setLinkId(connect18_ew.getId());
        ...
        scenario.getTransitSchedule().addStopFacility(stop19_facility_we);
        ...
        scenario.getTransitSchedule().addStopFacility(stop19_facility_ew);
        ...
        var Stop12_we = scheduleFactory.createTransitRouteStop(stop12_facility_we, 1380, 1390);
        var Stop13_we = scheduleFactory.createTransitRouteStop(stop13_facility_we, 1560, 1570);
        var Stop14_we = scheduleFactory.createTransitRouteStop(stop14_facility_we, 1800, 1810);
        var Stop15_we = scheduleFactory.createTransitRouteStop(stop15_facility_we, 1980, 1990);
        var Stop16_we = scheduleFactory.createTransitRouteStop(stop16_facility_we, 2160, 2170);
        var Stop17_we = scheduleFactory.createTransitRouteStop(stop17_facility_we, 2340, 2350);
        var Stop18_we = scheduleFactory.createTransitRouteStop(stop18_facility_we, 2520, 2530);
        var Stop19_we = scheduleFactory.createTransitRouteStop(stop19_facility_we, 2700, 2710);
        ...
        var Stop8_ew = scheduleFactory.createTransitRouteStop(stop8_facility_ew, 1320, 1330);
        var Stop9_ew = scheduleFactory.createTransitRouteStop(stop9_facility_ew, 1440, 1450);
        var Stop10_ew = scheduleFactory.createTransitRouteStop(stop10_facility_ew, 1560, 1570);
        var Stop11_ew = scheduleFactory.createTransitRouteStop(stop11_facility_ew, 1680, 1690);
        var Stop12_ew = scheduleFactory.createTransitRouteStop(stop12_facility_ew, 1800, 1810);
        var Stop13_ew = scheduleFactory.createTransitRouteStop(stop13_facility_ew, 1920, 1930);
        var Stop14_ew = scheduleFactory.createTransitRouteStop(stop14_facility_ew, 2040, 2050);
        var Stop15_ew = scheduleFactory.createTransitRouteStop(stop15_facility_ew, 2160, 2170);
        var Stop16_ew = scheduleFactory.createTransitRouteStop(stop16_facility_ew, 2340, 2350);
        var Stop17_ew = scheduleFactory.createTransitRouteStop(stop17_facility_ew, 2460, 2470);
        var Stop18_ew = scheduleFactory.createTransitRouteStop(stop18_facility_ew, 2580, 2590);
        var Stop19_ew = scheduleFactory.createTransitRouteStop(stop19_facility_ew, 2700, 2710);
        ...
        var networkRoute_we = RouteUtils.createLinkNetworkRouteImpl(startLink_we.getId(), List.of(connect1_we.getId(),connect2_we.getId(),
                ...
                connect15_we.getId(), connect16_we.getId(), connect17_we.getId(), connect18_we.getId()), endLink_we.getId());
        var route_we = scheduleFactory.createTransitRoute(Id.create("S13-we", TransitRoute.class), networkRoute_we,
                List.of(Stop1_we, Stop2_we, Stop3_we, Stop4_we, Stop5_we, Stop6_we, Stop7_we, Stop8_we,
                        Stop9_we, Stop10_we, Stop11_we, Stop12_we, Stop13_we, Stop14_we, Stop15_we, Stop16_we, Stop17_we, Stop18_we, Stop19_we),"pt");
        ...
        var networkRoute_ew = RouteUtils.createLinkNetworkRouteImpl(startLink_ew.getId(), List.of(connect1_ew.getId(),connect2_ew.getId(),
                ...
                connect15_ew.getId(), connect16_ew.getId(), connect17_ew.getId(), connect18_ew.getId()), endLink_ew.getId());
        var route_ew = scheduleFactory.createTransitRoute(Id.create("S13-ew", TransitRoute.class), networkRoute_ew,
                List.of(Stop1_ew, Stop2_ew, Stop3_ew, Stop4_ew, Stop5_ew, Stop6_ew, Stop7_ew, Stop8_ew,
                        Stop9_ew, Stop10_ew, Stop11_ew, Stop12_ew, Stop13_ew, Stop14_ew, Stop15_ew, Stop16_ew, Stop17_ew, Stop18_ew, Stop19_ew),"pt");
        ...
        new NetworkWriter(network).write(root.resolve("network-with-sbahn-complete-north.xml.gz").toString());
        new TransitScheduleWriter(scenario.getTransitSchedule()).writeFile(root.resolve("transit-Schedule-complete-north.xml.gz").toString());
        new MatsimVehicleWriter(scenario.getTransitVehicles()).writeFile(root.resolve("transit-vehicles-complete-north.xml.gz").toString());
    }
